...
1. **Create the network volume** for the HuggingFace cache to avoid re-downloading the ~3.4GB weights on every cold start. Mount it at `/runpod-volume/huggingface-cache`. Pre-warm it once:
