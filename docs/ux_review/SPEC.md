...
- **`last_verified_at.commit`** is the harness's last-green SHA (for `discovered_via: simulation | first-run`) or the manual verification commit (for human-only paths). Tracked metadata may use the literal `CURRENT_HEAD`; this marker is valid only with `last_verified_at.tree`, a SHA-256 fingerprint of the committed Git tree excluding the UX metadata files themselves. The verifier resolves it only when Git HEAD is available, the requested head is the actual checked-out HEAD, and the fingerprint still matches; later code/test/docs commits therefore require re-verification without a self-referential commit SHA.
