...
- Commit: `CURRENT_HEAD` (subject: `fix(phase-4d): close final acceptance blockers`).
- Evidence: focused credential, registration-transport, multipart, health-deadline, and retention-race regressions pass; `just validate` is the final gate with 1694 passing tests and 9 expected skips, with no pytest warnings.
