...
- Commit: `CURRENT_HEAD` (subject: `fix(phase-4d): enforce credential and response contracts`).
- Evidence: focused credential, registration-transport, multipart, health-deadline, and retention-race regressions pass; `just validate` is the final gate with 1683 passing tests and 9 expected skips, with no pytest warnings.
