...
**Goal:** Resolve all remaining open high- and medium-severity review stories in nine topology-ordered bundles, with up to five fresh review rounds per bundle and one atomic story commit after verification.

**Architecture:** Preserve existing core/shell/worker boundaries while repairing shared orchestration state, event lifecycle ownership, public schema boundaries, operational persistence, dashboard URL separation, worker resource handling, validation coverage, and developer gates. Each bundle is implemented by one worker, reviewed by fresh agents, corrected by one fix worker when needed, and then split into story-level commits by the parent.
...
- Commit each story separately with `fix(<STORY-ID>): <summary>` after the relevant code, tests, and review metadata are staged together.
...
- [ ] **Step 1: Add failing cache-boundary tests.**
...
- [ ] **Step 3: Implement the shared cache and canonical-root contract.**
...
- [ ] **Step 4: Run focused tests and inspect resource ownership.**
...
- [ ] **Step 6: After review-loop approval, stage code/tests and the three story entries; commit separately:**
...
- [ ] **Step 1: Add failing event-lifecycle tests.**
...
- [ ] **Step 3: Implement atomic terminal handling and bounded cleanup.**
...
- [ ] **Step 4: Run focused tests, including cancellation/disconnect paths.**
  - Re-run the command from Step 2 and verify no task or queue remains after the stream exits.
- [ ] **Step 5: Run `just validate`.**
- [ ] **Step 6: After review-loop approval, commit the four stories separately:**
...
- [ ] **Step 1: Add failing tests for warning exception classification.**
...
- [ ] **Step 2: Run `uv run pytest tests/shell/api/test_jobs.py -q` and confirm the unexpected error is currently swallowed.**
- [ ] **Step 3: Extract the four focused route modules without changing endpoint contracts.**
...
- [ ] **Step 4: Narrow BOOTING warning handling to the documented backend failure type and preserve exception chaining for unexpected errors.**
- [ ] **Step 5: Run the complete jobs route test file and verify submit, preview, retry, logs, cancel, resume, get, and list contracts.**
- [ ] **Step 6: Run `just validate`.**
- [ ] **Step 7: After review-loop approval, commit:**
...
- [ ] **Step 2: Run `uv run pytest tests/core/test_schemas.py tests/shell/api/test_schemas.py tests/shell/api/test_admin.py tests/shell/api/test_jobs.py tests/test_api_client.py -q` and confirm the metadata field is absent.**
- [ ] **Step 3: Move the cleanup response models to `core/schemas.py`, delete the server-local definition, and update imports.**
- [ ] **Step 4: Add the typed metadata field to `OutputSummary` and copy metadata in the response mapper without exposing internal-only fields.**
- [ ] **Step 5: Run focused tests, `just type-check`, and `just validate`.**
...
- [ ] **Step 1: Add failing exception-boundary tests.**
...
- [ ] **Step 2: Add failing audit durability tests.**
...
- [ ] **Step 3: Run `uv run pytest tests/shell/test_retention.py tests/shell/api/test_admin.py tests/shell/test_orchestrator.py -q` and confirm the broad catch and process-local audit behavior.**
- [ ] **Step 4: Narrow retention handling to `StoreError` with `raise ... from exc`; persist bounded audit records through the existing durable storage boundary or an append-only file under the canonical data root, and reload them on startup.**
- [ ] **Step 5: Run focused tests and `just validate`.**
- [ ] **Step 6: After review-loop approval, commit:**
...
- [ ] **Step 2: Run `uv run pytest dashboard/tests/test_dashboard.py dashboard/tests/test_job_detail.py dashboard/tests/test_cost_partial.py -q` and confirm the current link and two-query behavior.**
- [ ] **Step 3: Add the distinct browser-facing URL input with explicit validation and thread it only into rendered link context; keep internal fetch helpers on the internal URL.**
- [ ] **Step 4: Extend the cost summary route/service with a bounded job snapshot or a dedicated combined dashboard response, then update the partial to consume one response per poll.**
- [ ] **Step 5: Run focused dashboard and cost tests, then `just validate`.**
- [ ] **Step 6: After review-loop approval, commit:**
...
- [ ] **Step 1: Add failing pricing tests for successful refresh and unexpired cache paths.**
...
- [ ] **Step 2: Add a multipart regression test using a near-limit part and assert the handler receives the complete bytes while the parser uses file-backed/spooled storage rather than `BytesIO.getvalue()`.**
- [ ] **Step 3: Run `uv run pytest tests/worker_sdk/test_runpod_price.py tests/worker_sdk/test_edge_http_multipart.py -q` and confirm the pricing fall-through and memory-copy path.**
- [ ] **Step 4: Fix the estimate branch and replace the multipart accumulation/copy path with a bounded spooled temporary-file or equivalent file-backed input implementation; preserve size and metadata validation.**
- [ ] **Step 5: Run focused tests and `just validate`.**
- [ ] **Step 6: After review-loop approval, commit:**
...
- [ ] **Step 1: Add the intermediate-directory symlink test.**
...
- [ ] **Step 2: Run `uv run pytest tests/shell/api/test_job_outputs.py -q` and observe the new test fail only if the production path is not already protected.**
- [ ] **Step 3: Add parameterized malformed-WAV fixtures for non-PCM audio, zero byte rate, malformed RIFF header, and missing required chunks; assert the public domain error text for each branch.**
- [ ] **Step 4: Run `uv run pytest tests/shell/test_local_handlers.py -q` and ensure fixtures exercise the validators rather than only helper internals.**
- [ ] **Step 5: If the symlink or WAV tests expose a production defect, add the smallest production fix before changing expectations; otherwise keep this task test-only. Run `just validate`.**
- [ ] **Step 6: After review-loop approval, commit:**
...
- [ ] Run the broad whole-branch review against the range `$(git merge-base master HEAD)..HEAD` with correctness, tests, maintainability, API/security, performance, and documentation lenses.
- [ ] If the final review has concrete blockers, dispatch one fix worker with the complete finding set and run one scoped re-review.
- [ ] Run `just validate` on the final branch and inspect `git diff master...HEAD` directly.
- [ ] Confirm every selected story has `status: verified`, updated `files[].lines`, `last_verified_at`, and the story commit in `fixed_in`.
- [ ] Record any stale exclusions, parked review findings, or deferred optional improvements in the final handoff.
