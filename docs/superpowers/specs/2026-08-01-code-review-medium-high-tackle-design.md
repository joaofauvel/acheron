...
Address all 21 currently open high- or medium-severity code-review stories in topology-ordered bundles, with a worker/reviewer loop for each bundle and an atomic implementation commit per independently separable story or reviewed topology bundle.
...
8. Update matching review entries, then create an atomic `fix(<STORY-ID>): <summary>` commit per independently separable story; stories in one reviewed topology bundle may be grouped even when their file sets are disjoint, with all affected IDs represented in the review metadata. Dependent stories remain in dependency order.
