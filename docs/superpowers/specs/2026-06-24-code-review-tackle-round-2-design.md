...
## 10. Round 2 outcome (recorded by B25)

- **Total commits landed** (c2a80fc..2b01434, the integration-branch tip before B25): **65 commits** (7 bundle merges + 58 content commits; 47 new since the B07 baseline `9f9f3f5`).
- **Total bundles landed**: **7 of 25** — B08, B09, B13, B14, B16, B18, B22. **18 bundles remain** on the Round 2 plan (B10, B11, B12, B15, B17, B19, B20, B21, B23, B24, plus B25 which is this commit). **B24 was not done** in this round — the 3 broken-YAML stories (OBS-007, SEC-011, OBS-009) remain pending and will be fixed in a later round.
- **M-effort stories that needed iteration**: **0**. All 34 stories landed in B08–B22 landed as designed on the first correctness pass. The only mid-flight adaptation was B16's CORR-009 closure-to-class refactor (the per-step worker list was originally going to be a closure-captured value, but the implementation settled on a class-level `_invalidate_cache()` method on the step handler — purely local to the file, no design change).
- **Stories that were `partial` or `not-addressed` in the correctness pass**: **0** at the time of writing. The 34 stories have fixes landed + tests added; the **orchestrator's correctness subagent pass has not yet run** on this delta — the orchestrator will mark them `verified` after B25 lands, producing the final Round 2 tally (verified = 121, fixed = 3, open = 55).
- **Cross-cutting follow-up items for Round 3** (surfaced by the adversarial review of B08–B22):
  1. **ARCH-016 followup** — the B16 rename of `workers/_shared.py` → `workers/_shared_utils.py` was incomplete: 2 of the 4 Dockerfiles still reference the old name in their `COPY` directives (fixed in `69cf81b` per the followup commit, but Round 3 should re-audit any container build arg or compose override that might still point at the old path).
  2. **SEC-006 followup** — `SEC006.sanitise_exc_string` is currently applied to the `PlanResult.errors` strings and the edge `/execute` 500 body (B08), but `shell/executors/sequential.py`'s per-step `try/except` still formats the error into the step's `last_error` field without sanitising. Round 3 should add the sanitiser at that point too — the `last_error` field is now exposed via `/workers` (per SEC-010), so an unsanitised exception could leak via the read path even after B08's `/workers` scrub.
  3. **SEC-019 followup** — B08 fixed the `/execute` 500 body's `str(exc)` leak for the JSON-error path (SEC-012), but the **multipart branch** still uses `str(exc)` for its own error formatting. Round 3 should plumb `SEC006.sanitise_exc_string` through the multipart path as well; the SEC-019 story in B24 is the intended landing site.
  4. **OBS-003 followup** — the `job_id`/`request_id` contextvars introduced in B08 propagate through the orchestrator and into the RunPod error path, but the **integration test that pins the correlation** (i.e. asserts `job_id` appears in a log line that a test observed end-to-end) was not added. Add a `caplog`-based integration test that submits a job and asserts the matching `job_id` substring appears in at least one log line.
  5. **DATA-005 followup** — B22 added the corruption tests for `RedisWorkerStore._deserialize_worker` (missing status, invalid status string) but did not pin the **tolerant-fallback policy**: should the deserialiser raise `WorkerError` (reject the record) or coerce to a sentinel (`WorkerStatus.UNKNOWN`)? The story accepts both, but Round 3 should pick one to keep the test contract stable.
  6. **TEST-013 followup** — B22 added `X-Acheron-Metadata` round-trip tests for the edge HTTP path, but the test assertions don't cite the **specific line ranges** in `_edge_http.py` and the SDK multipart emitter where the header is set/parsed. Round 3's `code-review-update` should re-anchor `TEST-013`'s `files[].lines[]` to the post-B22 locations.
