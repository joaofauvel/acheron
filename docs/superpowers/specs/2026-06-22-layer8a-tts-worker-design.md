...
- **`workers.qwen3tts.handler.handle()` rejects non-list or empty `chunks` with an empty artifact list (not an error).** Plan body left that branch open. The orchestrator passes `chunks: []` for "no audio" jobs (e.g., empty book) — we return one default silent WAV in that case so the orchestrator's `streaming executor` always has at least one artifact to materialize. Tighter error semantics live in the model layer.
- **`Dockerfile.runpod` uses Python 3.12** (not 3.14). qwen-tts + flash-attn-2 wheels are pre-built for cp312; the 3.14-slim build is fine for the orchestrator but qwen-tts would need to compile from source on 3.14. The wheel-set is mature on 3.12. Dev tooling stays on 3.14.
- **`Dockerfile.edge` uses Python 3.14-slim** to match the orchestrator + dashboard images (smaller base image, single `apt` lineage).
...
- **The 7-stub matrix replaces the 4 legacy stubs** (worker_stub.py, grpc_worker_stub.py, translation_stub.py, plus the `tts-stub` / `asr-stub` / `translation-stub` / `tts-grpc-stub` compose services). New stubs:
  - `tts_local_stub`, `tts_volume_stub`, `tts_runpod_stub`, `tts_grpc_stub`, `asr_local_stub`, `translation_local_stub`, `translation_runpod_stub` — each a ~15-line `main.py` calling `create_worker_app` + `uvicorn.run`. Per-stub variance lives in `worker.yaml` (price_source, output_mode, listen_port, dollars_per_hour).
  - The `tts_grpc_stub` is HTTP-only at the edge (the gRPC contract is exercised in `tests/shell/test_grpc_worker.py` via `_FakeSynthesisServicer`). v1 workers ship HTTP, per the design.
  - The `tts_runpod_stub` and `translation_runpod_stub` start an in-process mock RunPod server on `127.0.0.1:8999` (and 8998 respectively). The mock honors the `ACHERON_WORKER__RUNPOD_BASE_URL` env hook so the SDK's `_runpod_client` can be redirected to it.
- **`tests/integration/test_tls.py` is xfailed.** The legacy stubs (`worker_stub`, `grpc_worker_stub`) had their own TLS plumbing (using `acheron.shell.tls.uvicorn_ssl_kwargs`); the SDK's `create_worker_app` is HTTP-only in v1. Worker-side TLS is deferred to a follow-up — orchestrator-side TLS is still covered by the SDK's TLS unit tests + the legacy cert path. The test will be restored when Plan 4 lands worker-side TLS.
- **`tests/integration/conftest.py` `grpc_tts_stub` fixture uses an inline `_LegacyGrpcTtsServicer`** (in the conftest) instead of the deleted `stubs.grpc_worker_stub.create_server`. The servicer emits legacy `pcm_data` chunks, exactly like the old stub did, so existing orchestrator-side gRPC tests don't have to be touched.
- **`stubs._sdk_base.StubTTSHandler` returns a single default silent WAV when `chunks` is empty/missing.** A stub job with no chunks still has to respond with a valid `JobResult.outputs`; the legacy `worker_stub` did the same. Translation stub returns `"mock translated text"` when no chunks.
- **`Justfile` adds `build-worker <name>` and `build-edge` targets** (replacing what the plan called `Makefile`-style targets). They wrap `uv build --package acheron --out-dir dist` + `docker build`; CI does the real publish.
...
- **Workspace dependency surface**: `workers/qwen3tts/pyproject.toml` declares `acheron` (workspace source), `qwen-tts`, and `soundfile` as runtime deps. `torch` + `flash-attn` are NOT declared as runtime deps in the workspace — they're installed directly by `Dockerfile.runpod`'s `pip install` against the cu121 index, so the workspace tests don't try to install GPU-only wheels on dev machines. `numpy~=2.5` and `soundfile~=0.14` are added to the root `dev` group so `workers/qwen3tts/tests/` can run without the worker package installed.
- **`mypy_path` and `basedpyright.extraPaths` now include `workers`** so the worker package is type-checked alongside the SDK. The import-linter has a new `workers-no-shell` contract (forbidden: `acheron.shell`).
- **`dev_certs` cert list updated for stubs** — the new `tts-stub` / `asr-stub` / `translation-stub` / `tts-grpc-stub` services share the existing cert names (no new certs needed; their TLS plumbing is xfailed, so TLS is moot for now).
