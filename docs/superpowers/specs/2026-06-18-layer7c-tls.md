...
**gRPC client (orchestrator → gRPC workers).** gRPC's Python API does not honor `SSL_CERT_FILE`. We have to wire the CA explicitly:
...
Each HTTP healthcheck is updated to read `SSL_CERT_FILE` and use it as the trust store. The gRPC stub's healthcheck (on port 9002) follows the same pattern.
...
- **New dev dep:** `cryptography~=44.0` (for the cert script). Not in the runtime image.
