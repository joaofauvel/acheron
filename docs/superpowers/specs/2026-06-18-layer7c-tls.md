...
**CLI (host → orchestrator HTTP).** The CLI's httpx client mirrors the same fallback rule: `ACHERON_TLS_CA_FILE` first, then `SSL_CERT_FILE`, otherwise the system trust store. The CLI's default `ACHERON_URL` is `https://localhost:8000` (so it Just Works against the dev/HTTPS orchestrator on the host). A user who wants HTTP-only sets `ACHERON_URL=http://localhost:8000` explicitly. The dashboard is unchanged and stays plain HTTP.
