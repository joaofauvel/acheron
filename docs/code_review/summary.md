...
5. **EXC-007** — Retention cleanup converts every job-store deletion error into an unlogged per-job failure [medium, S] — `code-quality.md`
6. **OBS-016** — Administrative audit events are process-local and never emitted to durable observability [medium, M] — `operations.md`
7. **PERF-013** — Dashboard cost polling performs two full job queries every two seconds [medium, M] — `operations.md`
8. **PERF-015** — Edge multipart parsing buffers each large input part and copies it again [medium, M] — `operations.md`
9. **TEST-031** — Nested output-directory symlink rejection lacks coverage [medium, S] — `verification.md`
10. **TEST-032** — PCM WAV rejection branches lack behavioral coverage [medium, S] — `verification.md`
...
7. **EXC-007** — narrow retention cleanup deletion failure handling [medium, S] — `code-quality.md`
8. **DOC-014** — document the actual administrative CLI namespaces [medium, S] — `surface.md`
9. **DX-009** — include UX rubric validation in `just validate` [medium, S] — `surface.md`
10. **CORR-047** — return measured RunPod pricing when a valid rate is available [medium, S] — `correctness.md`
...
The review was refreshed through `092ef77`, following the original `49747dd..22d20f5` scan. Since that scan, the medium/high tackle branch fixed cache propagation, job-event lifecycle cleanup, jobs-route decomposition, shared cleanup-schema ownership, and per-artifact output metadata; related citations, statuses, and summary counts were re-resolved. The remaining open and stale stories are listed above.
...
**Branch / HEAD**: `fix/code-review-medium-high` at `092ef77` (implementation commit; review-metadata follow-up may be newer).
